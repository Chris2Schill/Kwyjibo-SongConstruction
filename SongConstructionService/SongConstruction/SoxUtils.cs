        ...
        private static string BuildSoxMergeCommandArgs(double[] beatPattern, SoundClipInfo[] sounds, string outputFilepath)
        {
            ...
            if (beatPattern.Length > 3 && sounds.Length != 0)
            {
                for (int i = 3; i < beatPattern.Length; i += 2)
                {
                    ...
                    double beatOffset = Math.Abs((beatPattern[i+1] - 1) * secondsPerBeat); // We subtract 1 because the beatPattern is not zero-indexed and we want sounds on beat 1 to play immediately. We take the absolute value for cases where original beat offset < 1 and subtracting 1 makes it go negative
